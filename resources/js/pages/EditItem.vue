<template>
    <div>
        <router-link to="/Item" class="btn btn-primary my-2">
            Go back
        </router-link>
        ...
        <button @click="updated" type="submit" class="btn btn-primary">
            Submit
        </button>
    </div>
</template>
...
export default {
    ...
    async created() {
        ...
        this.weight = response2.data.data.weight;
        this.condition = response2.data.data.condition;
        this.store_id = response2.data.data.store_id;
    },
    methods: {
        async updated() {
            this.ok = null;
